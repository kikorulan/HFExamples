...
for i = 1:10:nSensors
    ...
    saveas(gcf, ['output_data/Example57_forwardSignal_sensor', int2str(i), '.fig']);
    saveas(gcf, ['output_data/Example57_forwardSignal_sensor', int2str(i)], 'png');
end
...
for i = 1:10:nSensors
    ...
    grid on;
    title(['RT vs kWave - ', int2str(i)]);
    saveas(gcf, ['output_data/Example57_forwardSignal_error', int2str(i), '.fig']);
    saveas(gcf, ['output_data/Example57_forwardSignal_error', int2str(i)], 'png');
end
%==========================================================================================
% INVERSE PROBLEM
%==========================================================================================
...
normRT = sortedNormRT(end-20);
inputRT_norm = inputRT/normRT;
% Plot
scrollView(permute(fliplr(permute(real(inputRT), [2 1 3])), [2 1 3]), 3, [0 normRT])
xlabel('y (m)');
ylabel('x (m)');
title('RT recon - z = 0.063');
saveas(gcf, 'output_data/Example57_RT_recon.fig');
...
% Normalisation - RT
sortedNormKW = sort(p0_recon(:));
normKW = sortedNormKW(end-20);

% Plot figure
scrollView(permute(fliplr(permute(real(p0_recon), [2 1 3])), [2 1 3]), 3, [0 normKW])
xlabel('y (m)');
ylabel('x (m)');
title('kWave recon - z = 0.063');
saveas(gcf, 'output_data/Example57_kWave_recon.fig');
