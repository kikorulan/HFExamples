...
%======================================================================
disp('GD');
GD.tau = {'2e1', '4e1', '8e1', '1.6e2', '3.2e2'};
GD.nIter = {30, 30, 30, 30, 23};
GD.lambda = '1e-4';
L = length(GD.tau);
...
for ii = 1:L
    ...
    for iter = 1:GD.nIter{ii}-1
        % Primal error
        ppmatrix = importdata(['./results_norm/adjoint/FB/pixelPressure_GD_tau', GD.tau{ii}, '_lambda', GD.lambda, '_iter', int2str(iter), '.dat'], ' ', 0);
        pp = matrix2cube(ppmatrix, Nz);
        pp_pos = max(0, pp);
        GD_error_pd{ii} = [GD_error_pd{ii} norm_distance(u0, pp_pos)];
        % Dual error
        tSignal = importdata(['./results_norm/forward/FB/forwardSignal_GD_tau', GD.tau{ii}, '_lambda', GD.lambda, '_iter', int2str(iter+1), '.dat'], ' ', 0);
        yi = tSignal(2:end, :);
...
ax = gca;
axis([0 50 0.7 20]);
legend('GD 2e1', 'GD 4e1', 'GD 8e1');
save ./results_norm/error_vectors/GD_error_lambda1em4 GD_error_pd GD_error_data GD_error_reg GD_error_dd GD;
...
%======================================================================
disp('SPDHG');
SPDHG.tau = {'1', '2', '5', '1e1', '2e1'};
SPDHG.sigma = '1';
SPDHG.nIter = {10, 10, 30, 24, 30};
SPDHG.lambda = '1e-4';
SPDHG.batch = '100';
L = length(SPDHG.tau);
...
for ii = 1:L
    ...
    for iter = 1:SPDHG.nIter{ii}-1
        % Primal error
        ppmatrix = importdata(['./results_norm/adjoint/SPDHG/pixelPressure_S-PDHG_sigma', SPDHG.sigma, '_tau', SPDHG.tau{ii}, '_theta1_lambda', SPDHG.lambda, '_batch', SPDHG.batch, '_subepoch', int2str(iter), '.dat'], ' ', 0);
        pp = matrix2cube(ppmatrix, Nz);
        pp_pos = max(0, pp);
        SPDHG_error_pd{ii} = [SPDHG_error_pd{ii} norm_distance(u0, pp_pos)];
        % Dual error
        tSignal = importdata(['./results_norm/forward/SPDHG/forwardSignal_S-PDHG_sigma', SPDHG.sigma, '_tau', SPDHG.tau{ii}, '_theta1_lambda', SPDHG.lambda, '_batch', SPDHG.batch, '_subepoch', int2str(iter+1), '.dat'], ' ', 0);
        yi = tSignal(2:end, :);
...
ax.GridAlpha = 0.2;
axis([0 30 0.7 20])
save ./results_norm/error_vectors/SPDHG_error_lambda1em4_sigma1_batch100 SPDHG_error_pd SPDHG_error_data SPDHG_error_reg SPDHG_error_dd SPDHG;

%========================================================================================================================
% PLOT ALL
%========================================================================================================================
load ./results_norm/error_vectors/GD_error_lambda1em4;
load ./results_norm/error_vectors/SPDHG_error_lambda1em4_sigma1;
...
hold on;       
loglog(0:SPDHG.nIter{SPDHG.plotIndex}-1, SPDHG_error_pd{SPDHG.plotIndex}, 'Color', 'm', 'Linewidth', 1.5);
legend('FB', 'SPDHG');
grid on;
box on;
ax = gca;
...
hold on;       
semilogy(0:SPDHG.nIter{SPDHG.plotIndex}-1, SPDHG_error_dd{SPDHG.plotIndex}, 'Color', 'm', 'Linewidth', 1.5);
legend('FB', 'SPDHG');
grid on;
box on;
ax = gca;
...
hold on;       
semilogy(1:SPDHG.nIter{SPDHG.plotIndex}-1, (SPDHG_error_dd{SPDHG.plotIndex}(1:end-1)-SPDHG_error_dd{SPDHG.plotIndex}(2:end))./SPDHG_error_dd{SPDHG.plotIndex}(2:end), 'Color', 'm', 'Linewidth', 1.5)
legend('FB', 'SPDHG');
grid on;
box on;
ax = gca;
...
hold on;       
semilogy(0:SPDHG.nIter{SPDHG.plotIndex}-1, SPDHG_error_data{SPDHG.plotIndex}, 'Color', 'm', 'Linewidth', 1.5);
legend('FB', 'SPDHG');
grid on;
box on;
ax = gca;
...
hold on;       
semilogy(0:SPDHG.nIter{SPDHG.plotIndex}-1, SPDHG_error_reg{SPDHG.plotIndex}, 'Color', 'm', 'Linewidth', 1.5);
legend('FB', 'SPDHG');
grid on;
box on;
ax = gca;
