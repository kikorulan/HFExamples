...
for n = 1:nIter
    ...
    % Save to disk
    filename = 'input_data/Example72_forward_input_A.h5';
    kspaceFirstOrder3D(kgrid, medium, source, sensor, input_args{:}, 'SaveToDisk', filename);
    % Call C++ code
    system('../kspaceFirstOrder3D-OMP -i input_data/Example72_forward_input_A.h5 -o output_data/Example72_forward_output_A.h5');
    ...
    % Read results
    sensor_data_adjoint = h5read('output_data/Example72_forward_output_A.h5', '/p');
    % Number of sensors
    ...
    % Save and run
    kspaceFirstOrder3D(kgrid, medium, source_adjoint, sensor_adjoint, input_args{:}, 'SaveToDisk', 'input_data/Example72_adjoint_input_A.h5');
    system('../kspaceFirstOrder3D-OMP -i input_data/Example72_adjoint_input_A.h5 -o output_data/Example72_adjoint_output_A.h5 --p_final');
    
    %==============================
    % UPDATE 
    %==============================
    p_PML = h5read('output_data/Example72_adjoint_output_A.h5', '/p_final');
    p = p_PML(1+PML_size:end-PML_size, 1+PML_size:end-PML_size, 1+PML_size:end-PML_size);
    % x_k
    x_k = conTVdenoising(y_k - 2*tau*p, lambda, para);
    x_k = max(0, x_k);
    % t_k
