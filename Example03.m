...
v = [floor(Nx/2) floor(2*Ny/3) u0; floor(Nx/3) floor(Ny/3) u0; floor(2*Nx/3) floor(Ny/3) u0];
...
% Run in 3 steps: each step of 1e-5 s
    ...
    view(2);
    colorbar;
    saveas(gcf, 'Example3_Backward-1.fig');

    % Second step: 2e-5
    gridIso.runFMM_Time(2e-5);
    h = figure;
    surf(x, y, gridIso.u, 'EdgeColor', 'none');
    view(2);
    colorbar;
    saveas(gcf, 'Example3_Backward-2.fig');
    ...
    view(2);
    colorbar;
    saveas(gcf, 'Example3_Backward-3.fig');
