...
for sim = 1:PDHG.length
    ...
    for iter = 1:5
        ppmatrix = importdata(['./results/pixelPressure_PDHG_sigma', PDHG.sigma{sim}, '_tau', PDHG.tau, '_theta', PDHG.theta, '_lambda', PDHG.lambda, '_iter', int2str(iter), '.dat'], ' ', 0);
        pp = max(0, matrix2cube(ppmatrix, Nz));
...
for sim = 1:SPDHG.length
    ...
    for iter = 1:5
        ppmatrix = importdata(['./results/pixelPressure_S-PDHG_sigma', SPDHG.sigma{sim}, '_tau', SPDHG.tau, '_theta', SPDHG.theta, '_lambda', SPDHG.lambda, '_batch', SPDHG.batch, '_epoch', int2str(iter), '.dat'], ' ', 0);
        pp = max(0, matrix2cube(ppmatrix, Nz));
